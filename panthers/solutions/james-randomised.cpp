...
void insert_line(int t, int b, int u, bool inc){ 
    // could even do this in log time as it's pretty easy, need to break even if this is log, but main soln is still randomised
    vector<int> line;
...
void insert_subtree(int rt, int u, bool inc = 0){
    ...
    if(!subtree.size()){
        ...
        int v = subtree[rand() % subtree.size()];
        int p = query(u,v);
